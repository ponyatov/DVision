...
public:
    ...
    /**
     * Returns a pointer to the menu item that has qtoupper(ch) as its hot key
     ...
     */
    TMenuItem *findItem( char ch ) const;
    /**
     ...
     */
    virtual TRect getItemRect( TMenuItem *item ) const;
    /**
...
private:
    ...
    Boolean mouseInOwner( const TEvent& e ) const;
    Boolean mouseInMenus( const TEvent& e );
    void trackMouse( const TEvent& e );
    TMenuView *topMenu();
    uchar updateMenu( const TMenu *menu );
...
public:
    ...
    static const TSymbol *frameChars;
private:

    void frameLine( TDrawBuffer&, short n ) const;
...
inline TStatusItem::TStatusItem( const char *aText,
    ...
    next( aNext ), text( aText ), keyCode( key ), command( cmnd )
{
}
...
private:
    ...
    void drawSelect( const TStatusItem *selected );
    void findItems();
    TStatusItem *itemMouseIsIn( const TPoint & );
    void disposeItems( TStatusItem *item ) const;
#ifndef NO_TV_STREAMS
    virtual const char *streamableName() const
        { return name; }
