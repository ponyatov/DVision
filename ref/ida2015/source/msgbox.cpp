...
static const char *const buttonName[] =
{
    MsgBoxText::yesText,
    MsgBoxText::noText,
    MsgBoxText::okText,
                       ...
                       ... )
{
    ...
    char msg[4096];
    qvsnprintf( msg, sizeof(msg), format, va );
    va_end( va );
...
ushort messageBox( ushort aOptions, const char *format, ... )
{
    va_list va;
    va_start( va, format );
...
{
    size_t L1 = ustrlen(aLabel) + 9 + limit;
    size_t L2 = ustrlen(Title) + 11;
    size_t len = qmax(L1, L2);
    len = qmin( len, 60 );
    len = qmax( len, 24 );
    TRect r(0, 0, len, 8);
    r.move((TProgram::deskTop->size.x - r.b.x) / 2,
