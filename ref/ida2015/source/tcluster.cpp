...
TCluster::~TCluster()
{
    //lint -e(1552) Converting pointer to array-of-derived to pointer to base
    destroy( (TCollection *)strings );
}

const char *TCluster::getItemText( ccIndex item ) const
{
...
void TCluster::drawBox( const char *icon, TSymbol marker)
{
    TDrawBuffer b;
    ushort color;
    ...
    for( int i = 0; i <= size.y; i++ )
    {
      for( int j = 0; j <= (strings->getCount()-1)/size.y + 1; j++ )
      {
        ...
        if ( cur < strings->getCount() &&
            col+ucstrlen(getItemText(cur))+5 < maxViewWidth &&
            col < size.x )
        {
      ...
      //lint -e(645) Symbol 'b' may not have been initialized
                ...
                for( int i = 0; i < strings->getCount(); i++ )
                    ...
                    if( getAltCode(c) == event.keyDown.keyCode ||
                        ( ( owner->phase == phPostProcess ||
                          ...
                          c != 0 &&
                          qtoupper(event.keyDown.charScan.charCode) == c
                        )
                      )
                        {
                        select();
...
int TCluster::findSel( const TPoint &p )
{
    TRect r = getExtent();
    if( !r.contains(p) )
        return -1;
