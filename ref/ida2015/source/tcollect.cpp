...
void *TNSCollection::at( ccIndex index ) const
{
...
void TNSCollection::atRemove( ccIndex index )
{
    ...
    count--;
    memmove( &items[index], &items[index+1], (count-index)*sizeof(void *) ); //lint !e671 possible negative arg
}
...
NORETURN void TNSCollection::error( ccIndex /*code*/, ccIndex )
{
    abort();
}

void *TNSCollection::firstThat( ccTestFunc Test, void *arg ) const
{
...
void *TNSCollection::lastThat( ccTestFunc Test, void *arg ) const
{
...
void TNSCollection::forEach( ccAppFunc action, void *arg ) const
{
    for( ccIndex i = 0; i < count; i++ )
        action( items[i], arg );
}
...
void TNSCollection::setLimit(ccIndex aLimit)
{
    ...
    if( aLimit != limit )
        ...
        if (aLimit == 0 )
            ...
            {
            aItems = OPERATOR_NEW(void *, aLimit);
            if( !aItems )
              return; // BF:TNSCollection-2
            if( count !=  0 )
