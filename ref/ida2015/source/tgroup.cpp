...
static void doCalcChange( TView *p, void *d )
{
    TRect  r;
...
static void addSubviewDataSize( TView *p, void *T )
{
...
void TGroup::drawSubViews( TView* p, const TView* bottom ) const
{
    while( p != bottom )
        {
        p->drawView();
...
void TGroup::getBuffer()
{
    if( (state & sfExposed) != 0 )
        if( (options & ofBuffered) != 0 && (buffer == 0 ))
            TVMemMgr::allocateDiscardable( (void * &)buffer, size.x * size.y * sizeof(TCharHolder) );
}
...
{
    return p->containsMouse( *(const TEvent *)s );
}
...
// This warning is wrong because if p->owner is not null, the view already
// belongs to someone
//lint -e{593} Custodial pointer 'p' (line 333) possibly not freed or returned
...
void TGroup::selectView( TView* p, Boolean enable ) const
{
    if( p != 0 )
        p->setState( sfSelected, enable );
}

void TGroup::focusView( TView* p, Boolean enable ) const
{
...
static void doExpose( TView *p, void *enable )
{
    if( (p->state & sfVisible) != 0 )
        p->setState( sfExposed, *(const Boolean *)enable );
}

struct setBlock
{
    ushort st;
    Boolean en; //lint !e958 padding is required to align members
};
...
//lint -e{818} Pointer parameter could be declared as pointing to const
static Boolean isInvalid( TView *p, void *cmdP )
{
