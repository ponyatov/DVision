...
void TMenuView::trackMouse( const TEvent& e )
{
...
Boolean TMenuView::mouseInOwner( const TEvent& e ) const
{
...
Boolean TMenuView::mouseInMenus( const TEvent& e )
{
...
TMenuItem *TMenuView::findItem( char ch ) const
{
    ch = qtoupper(ch);
    ...
    while( p != 0 )
        ...
        if( p->name != 0 && !p->disabled )
            ...
            if( loc != 0 && uchar(ch) == uchar(qtoupper(loc[1])) ) // ig 04.01.96
                return p;
            }
        p =  p->next;
        }
...
TRect TMenuView::getItemRect( TMenuItem * ) const
{
...
uchar TMenuView::updateMenu( const TMenu *_menu ) {
  if ( !_menu ) return False;
  uchar res = 2;
  for( TMenuItem *p = _menu->items; p != 0; p = p->next ) {
    if ( p->name != 0 ) {
      if ( p->command == 0 ) {
        uchar rrr = updateMenu(p->subMenu);
        res |= (rrr & 1);
