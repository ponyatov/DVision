...
void TStatusLine::disposeItems( TStatusItem *item ) const
{
    while( item != 0 )
        {
        TStatusItem *T = item;
...
void TStatusLine::drawSelect( const TStatusItem *selected )
{
    TDrawBuffer b;
    ushort color;
    char hintBuf[maxViewWidth];
    ...
    while( T != 0 )
        ...
        if( T->text != 0 )
            {
            ushort l = (ushort)ucstrlen( T->text );
            if( i + l < size.x )
                {
    ...
    if( i < size.x - 2 )
        {
        ustrncpy( hintBuf, hint( helpCtx ), sizeof(hintBuf) );
        if( hintBuf[0] != EOS )
            {
            b.moveStr( i, hintSeparator, cNormal );
            i += 2;
            if( ustrlen(hintBuf) + i > size.x )
...
TStatusItem *TStatusLine::itemMouseIsIn( const TPoint &mouse )
{
    if( mouse.y !=  0 )
        return 0;
    ...
    for( i = 0, T = items; T != 0; T = T->next)
        ...
        if( T->text != 0 )
            {
            ushort k = ushort(i + ucstrlen(T->text) + 2);
