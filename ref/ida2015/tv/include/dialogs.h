...
public:
    ...
    /**
     * Sets the read-only status of the control
     */
...
protected:
    Boolean canScroll( int delta ) const;
    int mouseDelta( const TEvent& event ) const;
    int mousePos( const TEvent& event ) const;
    void deleteSelect();
    void adjustSelectBlock();
    char *clip_get(size_t &clipsz) const;
    bool clip_put(void) const;

    static const TSymbol *rightArrow;
    static const TSymbol *leftArrow;
    int anchor;
...
protected:
    ...
    Boolean amDefault;
private:
    void drawTitle( TDrawBuffer&, int, int, ushort, Boolean ) const;
...
public:
     ...
     */
    void drawBox( const char *icon, TSymbol marker );
    virtual void getData( void *rec, size_t recsize );
    /**
...
private:
    ...
    int findSel( const TPoint &p );
    int row( int item ) const;
    const char *getItemText( ccIndex item ) const;
...
public:
    AS_PRINTF(3, 0) TParamText( const TRect& bounds, const char *aText, int aParamCount );
...
protected:
     ...
     */
#ifndef _HISTORY_NO_VEC
    TListViewer *(*createListViewer)( TRect r, TWindow *w, historyId_t histId, qstrvec_t *slist );
#else
    TListViewer *(*createListViewer)( TRect r, TWindow *w, historyId_t histId );
#endif
};
...
public:
     ...
     */
#ifndef _HISTORY_NO_VEC
    THistoryWindow( const TRect& bounds, historyId_t historyId, qstrvec_t *slist );
#else
    THistoryWindow( const TRect& bounds, historyId_t historyId );
#endif
    /**
     * Returns the default palette string.
     */
     ...
     * the list.
     */
    static TListViewer *initViewer( TRect bounds, TWindow *w, historyId_t
        aHistoryId
#ifndef _HISTORY_NO_VEC
        , qstrvec_t *slist
#endif
...
public:
     ...
     */
#ifndef _HISTORY_NO_VEC
    THistory( const TRect& bounds, TInputLine *aLink, historyId_t aHistoryId, qstrvec_t *slist = NULL );
#else
    THistory( const TRect& bounds, TInputLine *aLink, historyId_t aHistoryId );
#endif
    /**
...
protected:
     ...
     *
     * It is also possible to specify a qstrvec_t, instead of a history ID.
     */
    historyId_t historyId;
#ifndef _HISTORY_NO_VEC
    qstrvec_t *strlist;
