...
public:
    using TSortedCollection::indexOf;
    using TSortedCollection::insert;
...
public:
    ...
    void readDirectory( const char *dir, const char *wildCard, bool append = false );
    void readDirectory( const char *wildCard, bool append = false );
    void appendList( TFileCollection *aList );
...
const int
    ...
    fdNoCreateControls = 0x1000;   // Do not create controls in for creator. Call fill() later
...
public:
    ...
    TFileDialog( const char *aWildCard, const char *aTitle,
                 const char *inputName, ushort aOptions, historyId_t histId );
    ~TFileDialog();

    virtual void getData( void *rec, size_t recsize );
    void getFileName( char *buf, size_t bufsize ) const;
    virtual void handleEvent( TEvent& event );
    virtual void setData( void *rec );
...
protected:
    ...
    void createInput(int x, int y, const char *text);
    void createFileList(int x, int y, int h);
...
inline TDirEntry::TDirEntry( const char *txt, const char *_dir ) :
    displayText( newStr( txt ) ), directory( newStr( _dir ) )
{
}
...
public:
    using TCollection::indexOf;
    using TCollection::insert;
...
public:
    ...
    TChDirDialog( ushort aOptions, historyId_t histId, const char *title=NULL, const char *_inidir=NULL);
    virtual size_t dataSize();
    virtual void getData( void *rec, size_t recsize );
    virtual void handleEvent( TEvent& );
    virtual void setData( void *rec );
