...
// TView Help context codes
    ...
    hcDragging      = 1,
#ifdef __CDB__
    hcResizing      = 2,    // Resizing while move/resize
#endif
...
//  Event masks
    ...
    focusedEvents       = evKeyboard | evCommand | evMouseWheel;
...
inline void TCommandSet::operator += ( int _cmd )
{
    enableCmd( _cmd );
}
...
inline int TCommandSet::loc( int _cmd )
{
    return _cmd / 8;
}

inline int TCommandSet::mask( int _cmd )
{
    return masks[ _cmd & 0x07 ];
}
...
public:
    ...
    Boolean mouseInView( TPoint mouse ) const;
    Boolean containsMouse( const TEvent& event ) const;
    ...
    virtual void calcViewBounds( TRect& bounds, TPoint delta );
    ...
    void hideCursor();
    void drawHide( const TView *lastView );
    void drawShow( const TView *lastView );
    void drawUnderRect( const TRect& r, const TView *lastView );
    void drawUnderView( Boolean doShadow, const TView *lastView );
    ...
    static void disableCommands( const TCommandSet& commands );
    static void enableCommands( const TCommandSet& commands );
    ...
    static void setCommands( const TCommandSet& commands );
    ...
    TPoint makeGlobal( const TPoint &source ) const;
    TPoint makeLocal( const TPoint &source ) const;
    ...
    void ida_local writeLine( short x, short y, short w, short h, const TDrawBuffer& b );
    void TV_CDECL writeBuf(  short x, short y, short w, short h, const void *b );
    void TV_CDECL writeChar( short x, short y, TSymbol c, uchar color, short count );
...
#ifdef __UTF8__
    void TV_CDECL writeLineCnv( short x, short y, short w, short h, const TDrawBuffer& b );
#endif
...
private:
    ...
    void moveGrow( TPoint p,
                   TPoint s,
                   const TRect& limits,
                   const TPoint &minSize,
                   const TPoint &maxSize,
                   uchar mode
                 );
    void change( uchar mode, const TPoint &delta, TPoint &p, TPoint &s, uint32 ctrlState ) const;
...
inline void ida_local TView::writeLine( short x, short y, short w, short h,
                              const TDrawBuffer& b )
{
...
private:
     ...
     * Undocumented.
     */
    void TV_CDECL frameLine( TDrawBuffer& frameBuf, short y, short n, uchar color ) const;
    /**
     * Undocumented.
     */
 ...
 * @see TScrollBar::draw
 */
typedef TSymbol TScrollChars[5];
...
private:
     ...
     * Undocumented.
     */
    void showSBar( TScrollBar *sBar ) const;
...
public:
    ...
    void drawSubViews( TView *p, const TView *bottom ) const;
...
private:
    ...
    void focusView( TView *p, Boolean enable ) const;
    void selectView( TView *p, Boolean enable ) const;
...
public:
    ...
    TWindowInit( TFrame *(*cFrame)( const TRect &r ) );

protected:

    TFrame *(*createFrame)( const TRect &r );
...
public:
    ...
    virtual void close();
    virtual TPalette& getPalette() const;
    virtual const char *getTitle( short maxSize ) const;
    virtual void handleEvent( TEvent& event );
    virtual void calcViewBounds( TRect& bounds, TPoint delta );
    static TFrame *initFrame( const TRect &r );
    virtual void setState( ushort aState, Boolean enable );
    virtual void sizeLimits( TPoint& min, TPoint& max );
