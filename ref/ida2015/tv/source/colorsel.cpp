...
void TColorDisplay::draw()
{
    ...
    TDrawBuffer b;
    for( uint i = 0; i <= uint(size.x)/len; i++ )
...
{
    color = aColor;
    message( owner, evBroadcast, cmColorSet, (void *)(int)(*color) );
    drawView();
}
...
void TColorGroupList::focusItem( int item )
{
...
{
    uchar palsize = *pal->data + 1;
    size_t s = qmin(palsize, recsize);
