...
void THelpTopic::addCrossRef( TCrossRef ref )
{
    ...
    p =  OPERATOR_NEW(TCrossRef, numRefs+1);
    if (numRefs > 0)
        {
...
int THelpTopic::getNumCrossRefs() const
{
    return numRefs;
}
...
void THelpTopic::setCrossRef( int i, const TCrossRef& ref )
{
    TCrossRef *crossRefPtr;
...
char *THelpTopic::wrapText( char *text, int size,
          int& offset, Boolean wrap, char *line, size_t lineBufLen ) const
{
    int i;
    ...
    if ( (i > width) && wrap ) {
      ...
      if ( i > size ) {
        ...
        while( (i > offset) && !(qisspace((uchar)text[i])) )
          --i;
        if ( i == offset ) {
          i = offset + width;
          while ( (i < size) && !qisspace((uchar)text[i]) )
            ++i;
...
void THelpIndex::read( FILE *fp )
{
  if ( fread(&size, sizeof(size), 1, fp) == 1 )
  {
    ...
    else
    {
      ...
      for ( int i = 0; i < size; ++i )
      {
        int32 *indexArrayPtr = index + i;
        if ( fread(indexArrayPtr, sizeof(int32), 1, fp) != 1 )
          break;
      }
    }
  }
...
void THelpTopic::readCrossRefs( ipstream& s )
{
    ...
    s >> numRefs;
    crossRefs = OPERATOR_NEW(TCrossRef, numRefs);
    for (i = 0; i < numRefs; ++i)
        {
...
THelpTopic *THelpFile::getTopic( int i ) const
{
    int32 pos;
    THelpTopic *topic;
...
// this function should be provided by the UI
//lint -esym(526, ActionKey) Symbol not defined
...
static char *addxrefs( THelpTopic *topic, char *text, size_t textsize ) { /* ig 22.04.93 */
  ...
  while ( (text=strchr(text,'@')) != NULL ) {
      ...
      if ( end != NULL )
      {
        *end++ = '\0';
        ...
        if (resize > 0) // if the buffer needs resizing,
        {
          ...
          // resize it
          base_size += resize;
          base = (char*) qrealloc(base, base_size);
    ...
    if ( qsscanf(text,"0:%d",&y) != 1 ) continue;
...
THelpTopic *THelpFile::getTopic(int i) const
{
    ...
    while ( 1 )
    {
      ...
      int tmp;
      if ( qsscanf(text, "@%d:%d", &tmp, &i) != 2 )
        break;
      qfree(text); // iterate once more
    }
...
THelpTopic *THelpFile::invalidTopic() const
{
    THelpTopic *topic;
    TParagraph *para;
