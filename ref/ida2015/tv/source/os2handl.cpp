...
DWORD TThreads::controlKeyState = 0;
DWORD TThreads::evpending;
int TThreads::macro_playing = 0;
static int inited = 0;

//lint -esym(528, tvision_for_win32_starter) not referenced
static TThreads tvision_for_win32_starter;     // just to call resume()
...
static uchar get_alt_pressed_mask(void)
{
  ...
  if (alt_pressed_mask == 0) // initialize if not already done
  {
    ...
    if (!qgetenv("TV_IGNORE_RIGHT_ALT_PRESSED"))
...
unsigned char getShiftState() {
// returns a value that can be used as a substitute for the shift state at [0040:0017]
  uchar state = (uchar)TThreads::controlKeyState;
  uchar tvstate = 0;
...
static int event_type(const INPUT_RECORD &ir)
{
  if ( ir.EventType == MOUSE_EVENT )
    return IO_RAW_EVENT;

  // after focus changing controlKeyState is not valid
  if ( ir.EventType == FOCUS_EVENT) {
    TThreads::controlKeyState &= ~0xFF;
  }

  if ( ir.EventType == KEY_EVENT)
  {
    TThreads::controlKeyState = ir.Event.KeyEvent.dwControlKeyState;
...
static void switch_screen_size(HANDLE h, const COORD &from, const COORD &to)
{
  SMALL_RECT  r;
  uchar       chg;
...
bool TProgram::switch_screen(int to_user)
{
  ...
  static PCHAR_INFO           user_screen_buffer = NULL;
  static COORD                user_screen_size = { 0, 0 };
  static CONSOLE_CURSOR_INFO  user_cinfo;
  static COORD                user_cursorpos = { 0, 0 };
...
// in win7 'system autoconversion' from OEM don't work :(
bool TThreads::clipboard_put(const char *str, size_t from, size_t to)
{
    ...
    if ( to > from && OpenClipboard(NULL) )
    {
      ...
      hMem = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, sz+1);
      if ( hMem != NULL )
      {
        pc = GlobalLock(hMem);
        if ( pc == NULL ) GlobalFree(hMem);
        else
        {
          memcpy(pc, str, sz);
          ((char *)pc)[sz] = 0;
          OemToCharBuffA((char *)pc, (char *)pc, sz);
          GlobalUnlock(hMem);
          if ( SetClipboardData(CF_TEXT, hMem) ) res = true;
        }
      }
      hMem = GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, sizeof(wchar_t) *(sz+1));
      if ( hMem != NULL )
      {
        pc = GlobalLock(hMem);
        if ( pc == NULL ) GlobalFree(hMem);
        ...
        {
          MultiByteToWideChar(CP_OEMCP, MB_USEGLYPHCHARS, str, int(sz), (wchar_t *) pc, int(sz+1));
          ((wchar_t *)pc)[sz] = 0;
          GlobalUnlock(hMem);
          if ( SetClipboardData(CF_UNICODETEXT, hMem) ) res = true;
        }
      }
      CloseClipboard();
    }
    ...
    if ( hMem != NULL )
    {
      ...
      if ( clip != NULL )
      {
        ...
        if ( clsz != 0 )
        {
          if ( sz != 0 && sz < clsz )
            clsz = sz;
          answer = (char *)qalloc(clsz + 1);
          if ( answer != NULL )
          {
            char *p = answer;
            while ( *clip != 0 && p-answer < clsz )
            {
              *p = *clip++;
              if ( (uchar)*p < ' ' || *p == 127 )
              {
...
#pragma warning(disable:4996) // deprecated function _splitpath
#endif // _MSC_VER
