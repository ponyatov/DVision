...
static const ushort *const keyMap[] = { firstKeys, quickKeys, blockKeys };
...
int TEditor::charPos( size_t p, size_t target ) const
{
    int pos = 0;
    while( p < target )
    {
...
size_t TEditor::charPtr( size_t p, int target ) const
{
    int pos = 0;
#ifndef __UTF8__
...
Boolean TEditor::clipCopy()
{
    ...
    {
        if ( x11_clipboard_put(buffer, selStart, selEnd) )
        {
            selecting = False;
            update(ufUpdate);
            res = True;
...
{
#ifdef __NT__
    size_t csize = MAX_GET_FROM_CLIP;
    char *data = TThreads::clipboard_get(csize, false);
    if ( data != NULL)
    {
        insertBuffer( data, 0, csize, False, False );
        qfree(data);
    }
#else
#ifdef __LINUX__
    ...
    {
        size_t csize = MAX_GET_FROM_CLIP;
        char* data = x11_clipboard_get(csize, false);
        if ( data != NULL )
        {
            insertBuffer( data, 0, csize, False, False );
            qfree(data);
...
Boolean TEditor::cursorVisible() const
{
...
size_t TEditor::getMousePtr( const TPoint &m ) const
{
...
{
    if( (event.message.infoPtr == (void*)p) && (p->value != d) )
        {
        d = p->value;
        update( ufView );
        }
    ...
    switch( event.what )
        ...
        case evMouseDown:
            ...
            do  {
                ...
                setCurPtr(getMousePtr(event.mouse.where), select_mode);
                select_mode |= smExtend;
                unlock();
...
#else
                    insertText( &event.keyDown.utfCode, getUtf8Len(uchar(event.keyDown.utfCode)), False);
#endif
                ...
                default:
                    ...
                    switch( event.message.command )
                        ...
                        case cmCharRight:
                            ...
                            break;
                        case cmWordLeft:
                            setCurPtr(prevWord(curPtr), select_mode);
                            break;
                        case cmWordRight:
                            setCurPtr(nextWord(curPtr), select_mode);
                            break;
                        case cmLineStart:
                        ...
                        case cmLineDown:
                            ...
                            break;
                        case cmPageUp:
                            setCurPtr(lineMove(curPtr, -(size.y-1)), select_mode);
                            break;
                        case cmPageDown:
                            setCurPtr(lineMove(curPtr, size.y-1), select_mode);
                            break;
...
void TEditor::normalizeBuffer(void)
{
    setCurPtr(0, 0);
    char    *pi, *po;
    char    *bufEnd = &buffer[gapLen+bufLen];

    for (pi = &buffer[gapLen], po = buffer; pi < bufEnd; ) {
        if (*pi == '\n')
        {
            pi++;
            if (pi < bufEnd && *pi == '\r')
    ...
    memmove(&buffer[gapLen], buffer, bufLen);
}
