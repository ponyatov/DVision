...
{
#ifndef __UTF8__
    return qisalnum(ch) || ch == '_';
#else
    return qisalnum(ch) || ch == '_' || ((ch & 0x80) != 0);
#endif
}

static int countLines( void *buf, size_t count )
{
  int anzahl=0;
  char *str=(char*) buf;
...
size_t scan( const void *block, size_t size, const char *str )
{
  ...
  register const uchar*  testP= (const uchar*)block;
  register const uchar*  patP = (const uchar*)str;
  register int32 x = 1;
  ...
  testP = (const uchar*)block;
  const uchar* end = testP + testLength - patternLength;

  for( ; ; ) {

     if(testHash == patternHash)
         return testP-(const uchar*)block;
...
static size_t iScan( const void *block, size_t size, const char *str )
{
#ifndef __UTF8__
  const int32   q        = 33554393L;
  const int32   q32      = q<<5;
  ...
  for (i=0; i<patternLength; i++) {
    patternHash = ( (patternHash<<5) + qtoupper(*patP++)  ) % q;
    testHash    = ( (testHash   <<5) + qtoupper(*testP++) ) % q;
  }
  ...
  for( ; ; ) {
     ...
     // Advance & calculate the new hash value:
     testHash = ( testHash + q32 - qtoupper(*testP) * x                 ) % q;
     testHash = ( (testHash<<5)  + qtoupper(*(patternLength + testP++)) ) % q;
  }
#else
  register const char* testP= (const char*)block;
  size_t patternLength = ustrlen(str);
  const char* end = testP + size - patternLength;
  while (testP < end) {
      if (ustrnicmp(testP, str, patternLength) == 0)
         return testP-(const char*)block;
      testP += getUtf8Len(uchar(*testP));
  }
#endif
  return UINT_MAX;              // Not found.
}

Boolean TEditor::hasSelection() const
{
...
Boolean TEditor::isClipboard() const
{
    return Boolean(clipboard == this);
}

size_t TEditor::lineMove( size_t p, int count ) const
{
    size_t i = p;
    p = lineStart(p);
    int pos = charPos(p, i);
...
void TEditor::newLine()
{
    ...
    while( i < curPtr &&
         ...
         i++;
    insertText(crlf, sizeof(crlf)-1, False);
    if( autoIndent == True )
        insertText( &buffer[p], i - p, False);
}

size_t TEditor::nextLine( size_t p ) const
{
    return nextChar(lineEnd(p));
}

size_t TEditor::nextWord( size_t p ) const
{
    if (bufChar(p) == '\n') {
        p = nextChar(p);
        return p;
    ...
    while( p < bufLen && !isWordChar(bufChar(p)) && bufChar(p) != '\n') p = nextChar(p);
    return p;
}

size_t TEditor::prevLine( size_t p ) const
{
  return lineStart(prevChar(p));
}

size_t TEditor::prevWord( size_t p ) const
{
    if (bufChar(prevChar(p)) == '\n') {
        p = prevChar(p);
        return p;
    }
    while( p > 0 && isWordChar(bufChar(prevChar(p))) == 0 && bufChar(prevChar(p)) != '\n')
        p = prevChar(p);
...
Boolean TEditor::search( const char *substring, ushort opts )
{
    size_t pos = curPtr;
    size_t i;
    do  {
        if( (opts & efCaseSensitive) != 0 )
            i = scan( &buffer[bufPtr(pos)], bufLen - pos, substring);
        else
            i = iScan( &buffer[bufPtr(pos)], bufLen - pos, substring);
        ...
        if( i != sfSearchFailed )
            ...
            if( (opts & efWholeWordsOnly) == 0 ||
                !(
                    ...
                    ( i + strlen(substring) != bufLen &&
                        isWordChar(bufChar(i + strlen(substring)))
                    )
                 ))
                {
                lock();
                setSelect(i, i + strlen(substring), False);
...
void TEditor::setCmdState( ushort command, Boolean enable ) const
{
    TCommandSet s;
    s += command;
...
void TEditor::setCurPtr( size_t p, uchar select_mode )
{
    size_t anchor;
    if( (select_mode & smExtend) == 0 )
        anchor = p;
