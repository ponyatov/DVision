...
inline ushort _min( size_t u1, size_t u2 )
{
...
{
    // There is no memory leak because this function is called only once
    //lint -e(423) Creation of memory leak in assignment to ''
    buffer = new char[bufSize];
}

Boolean TFileEditor::loadFile()
{
  FILE *fi = qfopen(fileName, "rb");
  if( !fi )
  {
    setBufLen( 0 );
    return True;
  ...
  {
    qfseek(fi, 0, SEEK_END);
    off_t fSize = qftell(fi);
    qfseek(fi, 0, SEEK_SET);
    if( fSize > (int32)(UINT_MAX-0x1F) || setBufSize(fSize) == False )
    {
      qfclose(fi);
...
/*
      if ( fSize > INT_MAX )
      {
        ...
        fread( &buffer[bufSize - size_t(fSize) + INT_MAX], size_t(fSize - INT_MAX), 1, fi);
      }
      else
*/
      {
        qfread(fi, &buffer[bufSize - fSize], fSize);
      }
...
static void writeBlock( FILE *fo, const char *buf, unsigned len )
{
    while( len > 0 )
        {
...
#endif
        qunlink( backupName );
        qrename( fileName, backupName );
        }

    FILE *f = qfopen(fileName, "wb");
...
Boolean TFileEditor::setBufSize( size_t newSize )
{
    ...
    if( newSize != bufSize )
        ...
        memcpy( buffer, temp, _min( newSize, bufSize ) );
