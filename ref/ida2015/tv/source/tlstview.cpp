...
#ifndef __UTF8__
#define LIST_SEPARATOR  uchar(179)
#else
#define LIST_SEPARATOR  _UTF8_3(0x2502)
#endif
...
{
    int i, j, item;
    ushort normalColor, selectedColor, color, indent;
    ...
    indent = 0;
    if( hScrollBar != 0 )
        indent = ushort(hScrollBar->value);

    colWidth = short(size.x / numCols + 1);
    if (colWidth >= MAXSTR) colWidth = MAXSTR-1;    // for lint
    for( i = 0; i < size.y; i++ )
        {
        for( j = 0; j < numCols; j++ )
            {
...
#else
                getText( text, item, sizeof(text) );
#endif
                char buf[MAXSTR];
                buf[0] = '\0';
                if ( ustrlen(text) > indent ) {
                  ustrNcpy( buf, ustraddr(text, indent), colWidth );
                  *ustraddr(buf, colWidth) = EOS;
                }
            ...
            b.moveChar( curCol+colWidth-1, LIST_SEPARATOR, getColor(5), 1 );
            }
...
{
    if ( (first_selected | last_selected) < 0 )
        return isSelected(item) == True;
...
void TListViewer::setState( ushort aState, Boolean enable )
{
    ...
    if( getState(sfVisible) && (aState & sfSelected) != 0 )
    {
        if( hScrollBar != 0 )
        {
