...
void TView::calcViewBounds( TRect& bounds, TPoint delta )
{
...
void TView::disableCommands( const TCommandSet& commands )
{
...
void TView::change( uchar mode, const TPoint &delta, TPoint &p, TPoint &s, uint32 ctrlState ) const
{
...
void TView::drawHide( const TView *lastView )
{
    drawCursor();
    drawUnderView(Boolean(state & sfShadow), lastView);
}

void TView::drawShow( const TView* lastView )
{
    drawView();
    if( (state & sfShadow) != 0 )
...
void TView::drawUnderRect( const TRect& r, const TView *lastView )
{
...
void TView::drawUnderView( Boolean doShadow, const TView *lastView )
{
    TRect r = getBounds();
    if( doShadow != False )
        r.b += shadowSize;
...
void TView::enableCommands( const TCommandSet& commands )
{
    set_commandSetChanged(Boolean( commandSetChanged ||
...
ushort TView::getHelpCtx()
{
    if( (state & sfDragging) != 0 ) {
        ...
        return hcDragging;
#else
        return (getShiftState() & kbShift) ? hcResizing : hcDragging;
#endif
    }
    return helpCtx;
}
...
TPoint TView::makeGlobal( const TPoint &source ) const
{
...
TPoint TView::makeLocal( const TPoint &source ) const
{
    TPoint temp = source - origin;
    const TView *cur = this;
    while( cur->owner != 0 )
...
Boolean TView::mouseInView(TPoint mouse) const
{
...
void TView::setCommands( const TCommandSet& commands )
{
    set_commandSetChanged(Boolean( commandSetChanged ||
                                (curCommandSet != commands )));
    curCommandSet = commands;
...
Boolean TView::containsMouse( const TEvent& event ) const
{
