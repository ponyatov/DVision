...
TWindowInit::TWindowInit( TFrame *(*cFrame)( const TRect &r ) ) :
    createFrame( cFrame )
{
}
...
const char *TWindow::getTitle( short ) const
{
    return title;
}

void TWindow::setTitle(const char *newTitle)
{
    delStr( title );
...
void TWindow::calcViewBounds( TRect& bounds, TPoint delta )
{
    TView::calcViewBounds(bounds, delta);
    TView::calcViewBounds(zoomRect, delta);
}

TFrame *TWindow::initFrame( const TRect &r )
{
    return new TFrame(r);
}
